...
export default async function ProduccionPage() {
  ...
  const whereClause: any = {
    stage: {
      notIn: [
        "Entregado",
        "Post-Sale Follow-Up Pending (5 days)",
        "Post-Sale Follow-Up Pending (1 month)",
        "Cycle Closed",
      ],
    },
  };
  if (user.role === 'advisor') {
    whereClause.quotation = { salesAssociateId: user.id };
  }
      ...
      <div className="bg-white border border-[#D8D3CC] rounded-lg shadow-sm overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-[#F5F2EE]">
              ...
              return (
                <tr key={o.id} className={`hover:bg-[#F5F2EE]/50 transition-colors ${o.isPriority ? "bg-red-50/50" : ""}`}>
                  ...
                  <td className="px-6 py-4">
                    ...
                    <div className="text-xs text-[#8E8D8A] truncate max-w-[120px]">{o.deliveryMethod === "Store Pickup" ? "Recoger Tienda" : "Envío"}</div>
                  </td>
                  <td className="px-6 py-4">
                    {productionTimerActive ? (
                      <div className="flex flex-col items-center justify-center">
                        <div className="w-full bg-[#F5F2EE] rounded-full h-1.5 mb-1 relative overflow-hidden">
                          <div className={`h-1.5 rounded-full ${isOverdue ? "bg-red-500" : "bg-[#C5B358]"}`} style={{ width: `${Math.min(100, (daysElapsed / totalDays) * 100)}%` }}></div>
                        </div>
                      ...
                      <div className="flex justify-center text-[#8E8D8A] text-[10px] tracking-wider uppercase">Esperando confirmación</div>
                    ) : (
                  ...
                  <td className="px-6 py-4 text-right">
                    {(o.stage === "Producción" || o.stage === "Certificación") && o.isCertificatePending ? (
            ...
            {orders.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-[#8E8D8A]">No hay órdenes en producción.</td>
              </tr>
            )}
          </tbody>
        </table>
