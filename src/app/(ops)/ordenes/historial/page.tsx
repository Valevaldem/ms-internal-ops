...
export default async function PostVentaPage(props: {
  searchParams: Promise<{ tab?: string; salesChannel?: string }>;
}) {
  ...
  if (user.role === "advisor") whereClause.quotation = { ...(whereClause.quotation || {}), salesAssociateId: user.id };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        {user.role === "manager" && (
          <form className="flex w-full md:w-auto gap-2">
            ...
            {filterSalesChannel && (
              <Link href={`/ordenes/historial${tab === "archived" ? "?tab=archived" : ""}`} className="text-[#8E8D8A] hover:text-[#333333] flex items-center px-2 text-sm transition-colors">Limpiar</Link>
            )}
          </form>
        )}
      </div>
      ...
      {tab !== "archived" && (
        <div className="bg-[#F5F2EE]/80 border border-[#D8D3CC] text-[#333333] p-4 rounded-lg text-sm mb-6 flex gap-3 items-start shadow-sm">
          ...
          <div>
            <span className="font-semibold block mb-1 text-[#C5B358]">Recordatorio antes de Cerrar Ciclo:</span>
            El ciclo solo debe cerrarse después de que el cliente haya confirmado la correcta recepción de su pieza (mensaje, foto, reseña o encuesta).
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-[#D8D3CC] rounded-lg shadow-sm overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-[#F5F2EE]">
            {orders.map((o) => (
              <tr key={o.id} className="hover:bg-[#F5F2EE]/50 transition-colors">
                <td className="px-6 py-4">
                  <Link href={`/ordenes/${o.id}`} className="font-semibold text-[#333333] hover:text-[#C5B358] transition-colors">{o.quotation.folio || o.quotation.id.split("-")[0] + ".."}</Link>
                ...
                <td className="px-6 py-4">
                  ...
                  <div className="text-xs text-[#8E8D8A]">${o.quotation.finalClientPrice.toLocaleString("es-MX")}</div>
                </td>
                <td className="px-6 py-4 text-[#333333]">{o.deliveryMethod === "Store Pickup" ? "Tienda" : "Envío"}</td>
                <td className="px-6 py-4 text-center">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-semibold tracking-wider uppercase inline-block ${o.stage === "Cycle Closed" ? "bg-gray-100 text-gray-500" : "bg-blue-50 text-blue-600"}`}>{translateStage(o.stage)}</span>
                </td>
                <td className="px-6 py-4 text-right">
                  {o.stage !== "Cycle Closed" ? (
                    <form action={closeCycle} className="inline-block">
                      <input type="hidden" name="id" value={o.id} />
                      <button type="submit" className="text-xs text-[#C5B358] hover:text-[#333333] font-medium transition-colors">Cerrar Ciclo</button>
                    </form>
            ...
            {orders.length === 0 && (
              <tr><td colSpan={6} className="px-6 py-12 text-center text-[#8E8D8A]">No hay órdenes en esta sección.</td></tr>
            )}
          </tbody>
        </table>
      </div>
