...
function SidebarLink({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link href={href} className="flex items-center gap-3 px-4 py-2.5 rounded-md text-sm text-[#555555] hover:bg-[#F5F2EE] hover:text-[#333333] transition-colors w-full">
      <span className="text-[#8E8D8A]">{icon}</span>{label}
    </Link>
  );
}
export default async function OpsLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return (
    <div className="flex h-screen bg-[#F5F2EE] text-[#333333] font-sans antialiased">
      <aside className="w-64 bg-white border-r border-[#D8D3CC] flex flex-col items-center py-8 px-4 h-full shadow-sm">
        ...
        <nav className="flex-1 w-full space-y-2 overflow-y-auto">
          {user.role === 'certificate_operator' && (
            <>
              <div className="pt-2 pb-2"><p className="text-xs font-semibold text-[#8E8D8A] uppercase tracking-wider pl-4">Operación</p></div>
              <SidebarLink href="/certificados" icon={<Award size={18} />} label="Certificados" />
              <SidebarLink href="/certificados/manual" icon={<BookOpen size={18} />} label="Certs. Manuales" />
            </>
          )}
          {user.role !== 'certificate_operator' && (
            <>
              ...
              <div className="pt-4 pb-2"><p className="text-xs font-semibold text-[#8E8D8A] uppercase tracking-wider pl-4">Certificados</p></div>
              <SidebarLink href="/certificados" icon={<Award size={18} />} label="Certificados" />
              <SidebarLink href="/certificados/manual" icon={<BookOpen size={18} />} label="Certs. Manuales" />
            </>
          )}
        ...
        <div className="mt-auto w-full border-t border-[#D8D3CC] pt-4 flex flex-col gap-2 px-4">
          <div className="text-xs text-[#8E8D8A] uppercase tracking-wider font-semibold mb-1">Sesión Activa</div>
          <div className="flex flex-col gap-1 text-xs text-[#333333] bg-[#F5F2EE] p-3 rounded border border-[#D8D3CC]">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{user.role === 'manager' ? 'Manager' : user.role === 'certificate_operator' ? 'Certificadora' : 'Asesora'}</span>
            </div>
          </div>
          <form action={logout}>
